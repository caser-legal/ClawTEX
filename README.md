...
Customer install for the OpenClaw-style workspace and gateway in this repository. Copy the tree into a Kiro home you control. Nothing here should point at someone else's machine.

```bash
export KIRO_HOME="$HOME/.kiro"
mkdir -p "$KIRO_HOME"
# From a checkout of this repo. Adjust the source path to your clone.
rsync -a --exclude .git ./ "$KIRO_HOME/"
# Put the launcher on your PATH, or call it by path.
install -m 755 clawtex "$HOME/.local/bin/clawtex"
```

Set a Discord bot token only in your shell environment if you want that adapter. Do not commit it.

```bash
export DISCORD_BOT_TOKEN="REPLACE_WITH_DISCORD_BOT_TOKEN"
export DISCORD_CHANNEL_ID="REPLACE_WITH_CHANNEL_ID"
```

Requires `kiro-cli` on `PATH`. Gateway state (`gateway.pid`, `logs/`) is created on your machine at runtime and is not part of this tree.
...
### 4. Example Usage
```
You: "Fix the StoreKit trial eligibility in ExampleApp"
...
Agent: [Checks memory, sees StoreKit patterns, high confidence]
       ...
       @rowan: Reviewing the reference implementation in /opt/ClawTEX/reference/
       
       [Autonomous loop runs until complete]
       
       ✅ Fixed. Trial eligibility now uses isEligibleForIntroOffer.
       Verified against the reference app. Build passes, zero warnings.
```
...
- Knows the quality bar you document in the workspace
